...
if (isset($_POST['accion']) && $_POST['accion'] === 'asignarcalificaciones') {
...
if (isset($_POST['accion']) && $_POST['accion'] == 'asignarcalificaciones') { 
    // Aquí llamas la función que asigna las calificaciones
    $_SESSION['mensaje'] = asignarcalificaciones($_POST['id_usuario'], $_POST['id_materia'], $_POST['parcial1'], $_POST['parcial2'], $_POST['parcial3']);

    // Redirige a la misma página para evitar reenviar el formulario
             ...
             <div class="tarjeta">
                ...
                <table class="tabla">          
                    ...
                    <?php foreach ($alumnosAgrupados as $id => $alumno) { ?>
                        <tr>
                            <form method="POST" action="">
                                <!--Input oculto para la acción de asignar calificaciones-->
                                <input type="hidden" name="accion" value="asignarcalificaciones">
                                ...
                                <td><input type="number" name="parcial1" value="<?php echo $alumno['parcial_1']; ?>" min="0" max="10" step="0.1" required
                                 <?php  //Si ya hay una calificación, lo hacemos de solo lectura, 
                                        // y ponemos el mensaje de que se debe entrar a modificar
                                    if ($alumno['parcial_1'] != 0): ?>
                                    readonly 
                                    title="Entre a modificar calificaciones para cambiar calificación"
                                <?php endif; ?>></td>
                                ...
                                <?php //Validamos si el alumno ya tiene todas sus calificaciones asignadas
                                    $asignada1 = $alumno['parcial_1'] != 0;
                                    $asignada2 = $alumno['parcial_2'] != 0;
                                    $asignada3 = $alumno['parcial_3'] != 0;
                                    $todasAsignadas = $asignada1 && $asignada2 && $asignada3;?>
                                <td>
                                    <button class="boton-opcion" type="submit"
                                    <?php if ($todasAsignadas) echo 'disabled 
                                    title="Todas las calificaciones ya han sido asignadas."';?>>
                                    ASIGNAR</button>
                                </td>
                            </form>
                        </tr>
